...
@section('title', 'Create Employee | Share Fair')
@section('proxima')
...
<div class="employee-form-modern">
    ...
    <div class="page-header">
        <nav class="breadcrumb" aria-label="Breadcrumb">
            ...
            <span class="breadcrumb-separator" aria-hidden="true">/</span>
            <span class="breadcrumb-current">New Employee</span>
        </nav>
    </div>
    ...
    <div class="form-container">
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" aria-label="Create new employee">
            @csrf

            <div class="form-row">
                <div class="form-group">
                    <label for="name">Name <span class="required-asterisk" aria-hidden="true">*</span></label>
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required placeholder="Full name" autocomplete="name">
                </div>
                <div class="form-group">
                    <label for="email">Email <span class="required-asterisk" aria-hidden="true">*</span></label>
                    <input type="email" id="email" name="email" value="{{ old('email') }}" required placeholder="email@example.com" autocomplete="email">
                </div>
                <div class="form-group">
                    <label for="phone">Phone <span class="required-asterisk" aria-hidden="true">*</span></label>
                    <input type="tel" id="phone" name="phone" value="{{ old('phone') }}" required placeholder="Phone number" inputmode="tel">
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="checkbox-wrapper">
                                        <input type="checkbox" class="custom-checkbox permission-checkbox" id="perm-view-{{ $menu->id }}" name="permissions[{{ $menu->id }}][view]" value="1" aria-label="View {{ $menu->menu_name }}">
                                        <label for="perm-view-{{ $menu->id }}" class="checkbox-label"></label>
                                    </div>
                                </td>
                                <td>
                                    <div class="checkbox-wrapper">
                                        <input type="checkbox" class="custom-checkbox permission-checkbox" id="perm-create-{{ $menu->id }}" name="permissions[{{ $menu->id }}][create]" value="1" aria-label="Create {{ $menu->menu_name }}">
                                        <label for="perm-create-{{ $menu->id }}" class="checkbox-label"></label>
                                    </div>
                                </td>
                                <td>
                                    <div class="checkbox-wrapper">
                                        <input type="checkbox" class="custom-checkbox permission-checkbox" id="perm-edit-{{ $menu->id }}" name="permissions[{{ $menu->id }}][edit]" value="1" aria-label="Edit {{ $menu->menu_name }}">
                                        <label for="perm-edit-{{ $menu->id }}" class="checkbox-label"></label>
                                    </div>
                                </td>
                                <td>
                                    <div class="checkbox-wrapper">
                                        <input type="checkbox" class="custom-checkbox permission-checkbox" id="perm-delete-{{ $menu->id }}" name="permissions[{{ $menu->id }}][delete]" value="1" aria-label="Delete {{ $menu->menu_name }}">
                                        <label for="perm-delete-{{ $menu->id }}" class="checkbox-label"></label>
                                    </div>
                                </td>
                                <td>
                                    <div class="checkbox-wrapper">
                                        <input type="checkbox" class="custom-checkbox permission-checkbox" id="perm-upload-{{ $menu->id }}" name="permissions[{{ $menu->id }}][upload]" value="1" aria-label="Upload {{ $menu->menu_name }}">
